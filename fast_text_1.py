...
class FastText_tf(tf.keras.Model):
    """
    使用paddlepaddle框架编写
    ...
    def __init__(self, embedding_size,
        ...
        super(FastText_tf, self).__init__()
    ...
    def fit_data(self, data):
        ...
        self.subword2id_dict = {subword: id+1 for id, (subword, _) in enumerate(subwords_counter)}
        self.id2subword_dict = {id+1: subword for id, (subword, _) in enumerate(subwords_counter)}
        ...
        self.word2sub_dict = {id: sub + [0] * (max_length_word2sub - len(sub)) for id, sub in self.word2sub_dict.items()}
                ...
                for pos_w in pos_word:
                    ...
                    if batch_pin == batch_size:
                        if not test:
                            ...
                            center_word_batch = np.array([self.word2sub_dict[id] for id in center_word_batch]).astype(
                                'int64')
                            target_word_batch = np.squeeze(target_word_batch).astype('int64')
                            label_batch = np.squeeze(label_batch).astype('float32')
                            yield center_word_batch, target_word_batch, label_batch
                            batch = []
                            batch_pin = 0
                        else:
                            yield batch_pin
                    ...
                    while neg_num < self.neg_sampling_num:
                        ...
                        if batch_pin == batch_size:
                            if not test:
                                ...
                                center_word_batch, target_word_batch, label_batch = np.hsplit(batch, 3)
                                center_word_batch = np.squeeze(center_word_batch)
                                center_word_batch = np.array(
                                    [self.word2sub_dict[id] for id in center_word_batch]).astype('int64')
                                target_word_batch = np.squeeze(target_word_batch).astype('int64')
                                label_batch = np.squeeze(label_batch).astype('float32')
                                yield center_word_batch, target_word_batch, label_batch
                                batch = []
    ...
    def _init_embedding(self):
        ...
        self.subword_emb = layers.Embedding(input_dim=self.subvocab_size + 1,
        ...
        # embedding
        self.word_emb_hidden = layers.Embedding(input_dim=self.vocab_size,
                                                output_dim=self.embedding_size,
                                                embeddings_initializer=tf.keras.initializers.RandomUniform(
                                                    minval=-self.init_scale, maxval=self.init_scale),
                                                trainable=True,
    ...
    def get_embedding(self, word):
        ...
        :return:
        """
        emb = self.subword_emb.weight.numpy()
    ...
    def call(self, inputs, **kwargs):
        ...
        target_words_emb = tf.reduce_sum(target_words_emb, axis=1)
        center_words_emb = self.subword_emb(center_words)
        center_words_emb = tf.reduce_sum(center_words_emb, axis=1)
        # mul
        word_sim = center_words_emb * target_words_emb
        word_sim = tf.reduce_sum(word_sim, axis=-1)
        pred = tf.sigmoid(word_sim)
        return pred
...
if __name__ == '__main__':
    ...
    step = 0
    multiple = 100
    ft = FastText_tf(embedding_size=embedding_size, min_count=5, window_size=5, ngrams=(3, 4, 5, 6), neg_sampling_num=4,
                     skip_ratio=0.001)
    ft.compile(optimizer=tf.keras.optimizers.Adam(learning_rate),
              loss=tf.keras.losses.BinaryCrossentropy(),
              metrics=['accuracy'])
    data = ft.fit_data(x_all)
    total_x_num = 0
    total_steps = 0
    ...
    for epoch in range(epoch_num):
        ...
        for center, target, label in x:
            ...
            # split batches
            # fit
            ft.fit((center_batch, target_batch), label_batch, batch_size=batch_size, epochs=1)
            # show progress
            step += multiple
            if step % 1000 == 0:
