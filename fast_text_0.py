...
class FastText_pp(fluid.dygraph.Layer):
    """
    使用paddlepaddle框架编写
    ...
    def __init__(self, embedding_size,
        ...
        super(FastText_pp, self).__init__()
                ...
                for pos_w in pos_word:
                    ...
                    if batch_pin == batch_size:
                        if not test:
                            ...
                            center_word_batch = np.squeeze(center_word_batch).astype('int64')
                            ...
                            #target_word_batch = np.array([self.word2sub_dict[id] for id in target_word_batch]).astype('int64')
        ...
        if len(batch) > 0:
            if len(batch) == 1:
                if not test:
                    ...
                    '''yield np.array([batch[0][0]]).astype('int64'), \
                          np.array([self.word2sub_dict[batch[0][1]]]).astype('int64'), \
                          np.array([batch[0][2]]).astype('float32')'''
    ...
    def get_embedding(self, word):
        ...
        :return:
        """
        if self.sub_emb_to_array is None:
            self.sub_emb_to_array = self.subword_emb.weight.numpy()
    ...
    def forward(self, center_words, target_words, label):
        ...
        #center_words_emb = self.word_emb_hidden(center_words)
        #target_words_emb = self.subword_emb(target_words)
        #target_words_emb = fluid.layers.reduce_sum(target_words_emb, dim=1)
        # mul
...
if __name__ == '__main__':
    ...
    with fluid.dygraph.guard(fluid.CUDAPlace(0)):
        ft = FastText_pp(embedding_size=embedding_size, min_count=5, window_size=5, ngrams=(3, 4, 5, 6), neg_sampling_num=4,
                         skip_ratio=0.001)
        ft.train()
        data = ft.fit(x_all)
        total_x_num = 0
        ...
        for epoch in range(epoch_num):
            ...
            for center, target, label in x:
                ...
                for i in range(multiple):
                    ...
                    # transfer variable
                    center = fluid.dygraph.to_variable(center)
                    target = fluid.dygraph.to_variable(target)
                    label = fluid.dygraph.to_variable(label)
                    # loss propagation
                    pred, loss = ft(center, target, label)
                    loss.backward()
                    # adam target
                    if epoch == 0 and step == 0:
                        adam = fluid.optimizer.Adam(learning_rate=learning_rate, parameter_list=ft.parameters())
                    adam.minimize(loss)
                    # clear gradient
                    ft.clear_gradients()
                    # show progress
                    step += 1
                    ave_loss = (ave_loss * batch_size * step + loss.numpy()[0] * len_center) / (
                                batch_size * step + len_center)
                    if step % 10 == 0:
                        print("epoch {} - step {} / {} - ETA {:.0f}s - loss {:.6f} - ave_loss {:.6f}".format(epoch + 1,
                            str(step).rjust(len(str(total_steps))), total_steps, (time.time() - start) /
                            step * (total_steps - step), loss.numpy()[0], ave_loss))
